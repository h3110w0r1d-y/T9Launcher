    ...
    fun getPinYin(str: String): ArrayList<ArrayList<String>> {
        ...
        for (i in 0..<str.length) {
            ...
            if (pinyinArray != null) {
                ...
                for (j in 0..<size) {
                    if (pinyinSet.add(pinyinArray[j].substring(0, 1))) {
                        newList.add(letter2Number(pinyinArray[j].substring(0, 1)))
                    }
                    if (pinyinSet.add(pinyinArray[j])) {
                        newList.add(letter2Number(pinyinArray[j]))
                    }
                }
                result.add(newList)
                continue
